...
namespace SecureSocketProtocol3.Compressions
{
    ...
    public class UnsafeQuickLZ
    {
        ...
        private unsafe void FastWrite(UInt32 f, void* dst, UInt32 bytes)
        {
            if (BitConverter.IsLittleEndian)
            {
                switch (bytes)
        ...
        private unsafe Boolean MemCompare(void* m1, void* m2, UInt32 size)
        {
            Boolean match = true;
            for (UInt32 i = 0; match && i < size; ++i) match = ((Byte*)m1)[i] == ((Byte*)m2)[i];
            return match;
        }
        private unsafe Boolean MemCompare(void* m1, String str)
        {
            Byte[] source = Encoding.ASCII.GetBytes(str);
            fixed (Byte* m2 = source) return MemCompare(m1, m2, (UInt32)source.Length);
        }
        private unsafe void MemSet(void* dst, Byte val, UInt32 n) { for (UInt32 i = 0; i < n; ++i) ((Byte*)dst)[i] = val; }
        private unsafe void MemCopy(void* dst, void* src, UInt32 n) { for (UInt32 i = 0; i < n; ++i) *((Byte*)dst + i) = *((Byte*)src + i); }
        private unsafe void MemCopy(void* dst, String str)
        {
            Byte[] source = Encoding.ASCII.GetBytes(str);
            fixed (Byte* src = source) MemCopy(dst, src, (UInt32)source.Length);
        }
        private unsafe void MemCopyUP(Byte* dst, Byte* src, UInt32 n)
        {
            if (BitConverter.IsLittleEndian)
            {
                if (n < 5)
        ...
        private UInt32 GetCompressedSize(Byte[] source) { return GetHeaderField(source, 0, HeaderFields.COMPSIZE); }
        public UInt32 GetDecompressedSize(Byte[] source, UInt32 start) { return GetHeaderField(source, start, HeaderFields.UNCOMPSIZE); }
        ...
        private unsafe UInt32 UnsafeDecompress(Byte[] source, UInt32 start, Byte[] destination)
        {
            fixed (Byte* source_f = source, destination_c = destination)
            {
                Byte* source_c = source_f + start;
        ...
        public unsafe Byte[] compress(Byte[] source, uint offset, uint length)
        {
            ...
            if (used > 0)
            {
                Array.Resize(ref destination, (int)used);
                return destination;
            }
            return null;
        }
        public Byte[] decompress(Byte[] source, uint offset = 0)
        {
            ...
            if (size != 0)
            {
                ...
                UnsafeDecompress(source, offset, destination);
            }
            return destination;
        }
    }
